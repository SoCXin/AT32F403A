...
#define MPU_ADDRESS         0xD0    //从机地址  AD0接地，从机地址0x68,左移1位 为0xD0.
...
//u8 MPU6050_ReadOneByte(u16 ReadAddr);							//指定地址读取一个字节
//void MPU6050_WriteOneByte(u16 WriteAddr,u8 DataToWrite);		//指定地址写入一个字节
//void MPU6050_WriteLenByte(u16 WriteAddr,u32 DataToWrite,u8 Len);//指定地址开始写入指定长度的数据
//u32 MPU6050_ReadLenByte(u16 ReadAddr,u8 Len);					//指定地址开始读取指定长度数据
//void MPU6050_Write(u16 WriteAddr,u8 *pBuffer,u16 NumToWrite);	//从指定地址开始写入指定长度的数据
//void MPU6050_Read(u16 ReadAddr,u8 *pBuffer,u16 NumToRead);   	//从指定地址开始读出指定长度的数据
//u8 L_MPU6050_Check(void);
void Init_MPU_6050(void);		//初始化MPU6050
//void L_Init_MPU_6050(void);		//初始化MPU6050
void MPU6050_Init(void); //初始化IIC
